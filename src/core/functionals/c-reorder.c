...
REBNATIVE(reorder_p)  // see REORDER in %base-defs.r, for inheriting meta
{
  ...
  blockscope {
    ...
    // be pushed.
    //
    const RELVAL *item;  // starts as tail
    const RELVAL *at = VAL_ARRAY_AT(&item, ARG(ordering));
    for (; at != item--; ) {
        const REBCAN *canon = VAL_WORD_CANON(item);
        ...
        if (ANY_WORD(item)) {  // on the record, we only just allow WORD!...
            ...
            canon = VAL_REFINEMENT_CANON(item);
        ...
        Init_Any_Word_Bound(DS_PUSH(), REB_WORD, canon, exemplar, index);
    }
  ...
  cleanup_binder: {
    ...
    for (; key != tail; ++key, ++param, ++index) {
        ...
        //
        bool mentioned = (0 == Remove_Binder_Index_Else_0(&binder, canon));
        ...
        if (
            ...
            error = Error_No_Arg(label, KEY_CANON(key));
        }
    }
  }
