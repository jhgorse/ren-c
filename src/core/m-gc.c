...
// because with the redesigned "RL_API" in Revolt, ordinary REBSER nodes do
...
//
// Revolt has attempted to be increasingly miserly about bytes, and also
// added the ability for C extensions to hook the GC for a cleanup callback
// relating to HANDLE! for any non-Revolt types.  Hopefully this will reduce
...
// as any Revolt series/arrays/contexts/functions are done with full values.
//
...
// level Revolt services.  So ultimately their implementations would not
...
// Revolt's PAIR! uses a special kind of REBSER that does no additional memory
