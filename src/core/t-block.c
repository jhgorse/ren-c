...
//     CT_Lit_Path()
//
REBINT CT_Array(const REBCEL *a, const REBCEL *b, bool strict)
{
    ...
    RELVAL *aval = VAL_ARRAY_AT(a);
    RELVAL *bval = VAL_ARRAY_AT(b);
...
REB_R MAKE_Array(
    ...
    if (IS_INTEGER(arg) or IS_DECIMAL(arg)) {
        ...
        //
        // !!! Revolt unified MAKE and construction syntax, see #2263.  This is
        ...
        if (not VAL_VARARGS_PHASE(arg)) {
            ...
            REBVAL *param = SPECIFIC(
                ARR_HEAD(ACT_PARAMLIST(F_PHASE(param_frame)))
            );
