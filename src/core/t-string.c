...
//  CT_String: C
//
REBINT CT_String(const REBCEL *v1, const REBCEL *v2, bool strict)
{
    assert(ANY_STRING_KIND(CELL_KIND(v1)));
    assert(ANY_STRING_KIND(CELL_KIND(v2)));

    REBLEN l1  = VAL_LEN_AT(v1);
    REBLEN l2  = VAL_LEN_AT(v2);
    REBLEN len = MIN(l1, l2);

    REBCHR(const*) cp1 = VAL_STRING_AT(v1);
    REBCHR(const*) cp2 = VAL_STRING_AT(v2);
