...
//  CT_Context: C
//
REBINT CT_Context(const REBCEL *v1, const REBCEL *v2, bool strict)
{
    assert(ANY_CONTEXT_KIND(CELL_KIND(v1)));
    assert(ANY_CONTEXT_KIND(CELL_KIND(v2)));

    if (CELL_KIND(v1) != CELL_KIND(v2))  // e.g. ERROR! won't equal OBJECT!
        return CELL_KIND(v1) > CELL_KIND(v2) ? 1 : 0;
