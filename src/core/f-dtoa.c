//
// This file was downloaded from: http://www.netlib.org/fp/dtoa.c
//
// It contained two functions, `dtoa()` and `strtod()`.  The latter function
// was removed in R3-Alpha--it's part of a family of functions that are
// officially part of the C99 standard (and was apparently widely available
// before that time):
//
// https://en.cppreference.com/w/c/string/byte/strtof
//
// If we kept it, we'd be getting multiple symbol definitions in the linker.
// There is currently no platform being built for that does not have strtod(),
// so if you wnat to build for one you'll have to reinstate the code somewhere.
//
// The copyright date on this file is a bit old.  An updated version does
// exist dated 2006, but the build process for it fragments it into many files
// and somewhat runs against Rebol's single-file-ethos.  Bringing it into the
// build properly would involve doing something like the zlib extraction, to
// consolidate it into a single file via an automatic process--vs. adding a
// build and configuration step for all clients.  TBD.
//
// For those interested, here's some reading about the nature of this file:
//
...
//=//// THIS IS WHERE `strtod()` WAS REMOVED ///////////////////////////////=//
//
// !!! See notes at top of file for why it was taken out.  Other functions
// that were removed since only strtod() used them:
//
// * s2b()
// * b2d()
// * ratio()
...
// * ulp()
//
// This probably leaves behind some stray definitions.  If anything takes up
// space it should be found and deleted.
//
//=/////////////////////////////////////////////////////////////////////////=//
