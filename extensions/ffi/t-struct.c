...
// !!! This is a non-stackless REDUCE operation, which is deprecated.  The
// FFI code should be rewritten to do more of its pre-processing in usermode,
// in order to get stacklessness "for free"...and just do the basic system
// interface part in the C code.  See the TCC module for an example of how
// this separation was approached.
//
static bool Reduce_To_Stack_Throws(
    REBVAL *out,
...
REB_R MAKE_Struct(
    ...
    Push_Frame(nullptr, f, &Evaluator_Executor);
...
//
    ...
    while (NOT_END(f_value)) {
        ...
        if (IS_END(f_value) or not IS_BLOCK(f_value))
            fail (Error_Bad_Value_Core(f_value, VAL_SPECIFIER(arg)));
        ...
        if (expect_init) {
            ...
            if (IS_BLOCK(f_value)) {
                ...
                if (Reduce_To_Stack_Throws(out, f_value, VAL_SPECIFIER(arg)))
...
//  CT_Struct: C
//
REBINT CT_Struct(const REBCEL *a, const REBCEL *b, bool strict)
{
    if (strict)
